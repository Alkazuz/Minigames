...
import br.alkazuz.vip.run.FallDamageManager;
...
public class MathUtils {
    ...
    static public float PI = 3.141592653589793238462643383279f;

    static public float PI2 = PI * 2;

    static public float SQRT_3 = 1.73205080757f;

    static public float E = 2.7182818284590452354f;
    ...
    static private int ATAN2_BITS2 = ATAN2_BITS << 1;

    static private int ATAN2_MASK = ~(-1 << ATAN2_BITS2);

    static private int ATAN2_COUNT = ATAN2_MASK + 1;

    static int ATAN2_DIM = (int) Math.sqrt(ATAN2_COUNT);

    static private float INV_ATAN2_DIM_MINUS_1 = 1.0f / (ATAN2_DIM - 1);
...
// .longBitsToDouble(NumberUtils.doubleToLongBits(BIG_ENOUGH_INT + 1) - 1);
    ...
    public static void applyVelocity(Entity ent, Vector v) {
        if (ent.hasMetadata("NPC"))
            return;
        ent.setVelocity(v);
        Bukkit.getScheduler().runTaskLaterAsynchronously(Main.theInstance(), () -> FallDamageManager.addNoFall(ent), 5);
    }

    public static void applyVelocity(Entity ent, Vector v, boolean ignoreGadgetsEnabled) {
        if (ent.hasMetadata("NPC"))
            return;
        ent.setVelocity(v);
        Bukkit.getScheduler().runTaskLaterAsynchronously(Main.theInstance(), () -> FallDamageManager.addNoFall(ent), 4);
    }

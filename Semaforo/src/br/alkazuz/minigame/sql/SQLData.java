...
public class SQLData
{
    ...
    public static void CriarTabela() {
        try {
            PreparedStatement ps = SQLData.con.prepareStatement("CREATE TABLE IF NOT EXISTS `"+MinigameConfig.NAME.toLowerCase().replace(" ", "_")+"_data` (`nick` VARCHAR(35) NULL,`object` TEXT NOT NULL);");
            ps.executeUpdate();
        }
        catch (Exception ex) {
            ex.printStackTrace();
